...
proc tup*(v: Vec2): (float32, float32) =
  (v.x, v.y)

proc tup*(v: Vec3): (float32, float32, float32) =
  (v.x, v.y, v.z)
...
proc init*(
    T: type Layer, w = screenWidth(), h = screenHeight(), depth = 0, camera = false
): T =
  T(depth: depth, camera: camera, target: Canvas.init(screenWidth(), screenHeight()))
...
proc drawLine*(
    startX, startY, endX, endY: SomeNumber, color = color(1.0, 1.0, 1.0, 1.0)
) =
...
proc drawRect*(
    pos, size: Vec2, origin = vec2(), rotation = 0.0, color = color(1.0, 1.0, 1.0, 1.0)
) =
...
proc drawLinerect*(
  ...
  application.linerect(
    x.float32, y.float32, w.float32, h.float32, origin, rotation, color
  )
...
template layer*(artist: var Artist, depth: int, body: untyped, clear = true) =
  let layer = artist.getOrCreateLayer(depth)
  startCanvas(layer.target)
  if clear:
    clearCanvas(layer.target)
  containerSize = layer.size()
  body
  endCanvas()

template cameraLayer*(artist: var Artist, depth: int, body: untyped, clear = true) =
  let layer = artist.getOrCreateLayer(depth)
  startCanvas(layer.target)
  if clear:
    clearCanvas(layer.target)
